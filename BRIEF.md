...
**What to call this, precisely.** You're wiring a *fixed* roster — the same four specialists fire on every RFP. In Anthropic's [Building Effective Agents](https://www.anthropic.com/engineering/building-effective-agents) vocabulary that's **parallelization (sectioning)** feeding a **synthesis** step — *not* the *orchestrator-workers* pattern, which decides its subtasks dynamically at runtime based on the input. A fixed roster is the right call for a 40-minute build with a live defense right after: it's reliable, and it always demos. Want the coordinator to actually *choose* its specialists per RFP? That's stretch goal S12 — and it reintroduces nondeterminism on purpose, so read the tradeoff before you reach for it.
...
6. **Verify the deliverable.** `python check_deliverable.py` — the required check, and the habit that matters: a pipeline that *ran* is not a proposal that's *right*. No API call, no second model — a fast local read of the docx. It gates on a scenario-agnostic **floor** (a real, assembled deliverable exists), and — while you're on the wired Acme RFP — grades the **Great when** bar for free (sections, past wins, SLA honesty, overpromises) as advisory guidance. **Adapting the scenario? The check comes with you:** drop an `acceptance.txt` next to the script — one criterion per line (plain text, `/regex/`, or `!must-not`) — and it grades *your* build against *your* bar. Every ✗ is a section to reopen; the lever is always a prompt or a skill, so change the specialist and re-run `create_coordinator.py` → `run_deal_desk.py` → `check_deliverable.py`.

By the end you have `outputs/proposal-response.docx`, generated by a coordinator plus specialists who each used their own skill — and a check that says it actually holds up.
...
- **0–10 — wiring.** `check_setup.py`, then the five build scripts in order. Get the docx out *once*, exactly as shipped, before you change a thing. A working baseline is worth more than a clever half-build.
- **10–28 — build.** Iterate on what the run actually produces. Your knobs are the **prompts** (the specialist system prompts in `create_specialists.py`, the coordinator prompt in `create_coordinator.py`) and the **skills** (the SKILL.md files). Change a specialist → re-run `create_coordinator.py` → re-run the deal. Cut scope before you cut the demo: two specialists handing off cleanly beats four wired badly.
- **28–33 — grade and tighten.** `python check_deliverable.py`. It grades the doc against the **Great when** bar in ~2 seconds (no API call). Fix the loudest ✗ — usually Pricing not citing a past win, or Technical Fit overselling the SLA — then re-run the deal once more. This is the step that turns "it ran" into "it holds up in the defense."
- **33–40 — lock the demo.** One clean end-to-end run. Events stream scrolling in one window, `proposal-response.docx` open in another, `check_deliverable.py` green in a third.

## Done when / Great when

**Done when:** `outputs/proposal-response.docx` exists, the events stream showed **four specialist threads spawn and run in parallel** before the coordinator synthesised them, **and `python check_deliverable.py` passes** — its scenario-agnostic floor (a real, assembled deliverable) plus any `acceptance.txt` criteria you've set.

**Great when:** on the wired Acme scenario, `check_deliverable.py`'s advisory grade comes back green on all three quality criteria (and if you've adapted, your own `acceptance.txt` passes):
...
When you adapt, the verification habit comes with you: write what "good" looks like for the new scenario into an `acceptance.txt` (one criterion per line — plain text, `/regex/`, or `!must-not`) and `check_deliverable.py` grades your build against *your* bar instead of the wired one. That's the same muscle you'll use in Drive Value — name the criteria, prove you hit them.

## Stretch goals

Want to push further? [`stretch-goals.md`](./stretch-goals.md) has the menu — codify *your* firm's voice as a skill, add a Critic sub-agent that reviews the draft before it ships (a real second LLM pass — the ceiling above `check_deliverable.py`'s floor), give the coordinator memory across deals, let it choose its own roster per RFP (S12), or have the swarm grade its own deliverable against a rubric (Outcomes).
