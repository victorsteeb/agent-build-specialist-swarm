...
- **Wiring.** `check_setup.py`, then the five build scripts in order. Get the docx out *once*, exactly as shipped, before you change a thing. A working baseline is worth more than a clever half-build.
- **Build.** Iterate on what the run actually produces. Your knobs are the **prompts** (the specialist system prompts in `create_specialists.py`, the coordinator prompt in `create_coordinator.py`) and the **skills** (the SKILL.md files). Change a specialist → re-run `create_coordinator.py` → re-run the deal. Cut scope before you cut the demo: two specialists handing off cleanly beats four wired badly.
- **Grade and tighten.** `python check_deliverable.py`. It grades the doc against the **Great when** bar in ~2 seconds (no API call). Fix the loudest ✗ — usually Pricing not citing a past win, or Technical Fit overselling the SLA — then re-run the deal once more. This is the step that turns "it ran" into "it holds up in the defense."
- **Lock the demo.** One clean end-to-end run. Events stream scrolling in one window, `proposal-response.docx` open in another, `check_deliverable.py` green in a third.
